...
/**
 * Set a flag as governed by a specific pval
 *
 * The bitflag identified by `flag` is set in `flags` under the pval
...
bool flag_set_pval(bitflag *flags, const size_t size, int flag, int pval)
{
	...
	assert(flag_offset < size);
	assert(pval <= MAX_PVALS);

	if (flags[flag_offset][pval] & flag_binary) return FALSE;

	flags[flag_offset][pval] |= flag_binary;

	return TRUE;
}

/**
 * Test if a particular flag is governed by a particular pval
 *
...
bool flag_test_pval(bitflag *flags, const size_t size, int flag, int pval)
{
	...
	if (flags[flag_offset][pval] & flag_binary) return TRUE;

	return FALSE;
}

/**
 * Clear a flag from being governed by a specific pval
 *
 * The bitflag identified by `flag` is cleared in `flags` under the pval
 * identified by 'pval'. The bitfield size is supplied in `size`.  
 * TRUE is returned when changes were made, FALSE otherwise.
 */ 
...
/**
 * Test to see whether any of an object's pvals have this flag set.
 *
...
int flag_test_all_pvals(bitflag *flags, const size_t size, int flag)
{
	...
	{
		if (flags[flag_offset][i] & flag_binary)
			return i;
	}
