...
static void adjust_level(struct player *p, bool verbose)
{
	...
	while ((p->lev < PY_MAX_LEVEL) &&
		...
		effect_simple(EF_RESTORE_STAT, source_none(), "0", STAT_STR, 0, 0, NULL);
		effect_simple(EF_RESTORE_STAT, source_none(), "0", STAT_INT, 0, 0, NULL);
		effect_simple(EF_RESTORE_STAT, source_none(), "0", STAT_WIS, 0, 0, NULL);
		effect_simple(EF_RESTORE_STAT, source_none(), "0", STAT_DEX, 0, 0, NULL);
		effect_simple(EF_RESTORE_STAT, source_none(), "0", STAT_CON, 0, 0, NULL);
	}
