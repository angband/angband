...
bool player_has_temporary_brand(int idx)
{
	...
	while (i < TMD_MAX) {
		if (timed_effects[i].brand_dup && player->timed[i] &&
				streq(brands[idx].code,
				timed_effects[i].brand_dup)) {
			return true;
		}
		++i;
	}
...
bool player_has_temporary_slay(int idx)
{
	...
	while (i < TMD_MAX) {
		if (timed_effects[i].slay_dup && player->timed[i] &&
				streq(slays[idx].code,
				timed_effects[i].slay_dup)) {
			return true;
		}
		++i;
	}
