...
bool borg_check_LIGHT(void)
{
    ...
    if (borg_skill[BI_MAXCLEVEL] > 10 && !borg_skill[BI_CDEPTH])
        return (false);
    ...
    /* if (!when_detect_evil ||
       (borg_t - when_detect_evil  >= 183 - (80 - borg_skill[BI_MAXCLEVEL])))
       do_evil = true; */
    ...
    /* Hack -- find evil */
    if (do_evil && (!when_detect_evil || (borg_t - when_detect_evil >= 20))) {
        /* Check for evil */
    ...
    /* Hack -- find objects */
    if (do_obj && (!when_detect_obj || (borg_t - when_detect_obj >= 20))) {
        /* Check for objects */
...
static bool borg_enchant_to_a(void)
{
    ...
    if ((!borg_skill[BI_AENCH_ARM]) && (!borg_skill[BI_AENCH_SARM]))
        return (false);
...
static bool borg_enchant_to_d(void)
{
    ...
    /* Need "enchantment" ability */
    if ((!borg_skill[BI_AENCH_TOD]) && (!borg_skill[BI_AENCH_SWEP]))
        return (false);
    ...
    if (borg_skill[BI_FIRST_CURSED]) {
        ...
        if (borg_read_scroll(sv_scroll_remove_curse)
            ...
            if (borg_skill[BI_FIRST_CURSED] <= INVEN_WIELD) {
                borg_keypress(
                    all_letters_nohjkl[borg_skill[BI_FIRST_CURSED] - 1]);
            } else if (borg_skill[BI_FIRST_CURSED] <= QUIVER_START) {
                if (borg_skill[BI_WHERE_CURSED] & BORG_INVEN)
                    borg_keypress('/');

                borg_keypress(all_letters_nohjkl[borg_skill[BI_FIRST_CURSED]
                                                 - INVEN_WIELD - 1]);
            } else {
                if (borg_skill[BI_WHERE_CURSED] & 1
                    || borg_skill[BI_WHERE_CURSED] & BORG_EQUIP)
                    borg_keypress('|');
                borg_keypress(
                    '0' + (borg_skill[BI_FIRST_CURSED] - 1 - QUIVER_START));
            }
            /* pick first curse */
            borg_keypress(KC_ENTER);
...
bool borg_enchanting(void)
{
    ...
    if ((borg_t - borg_began > 5500 && borg_skill[BI_CDEPTH] >= 1)
        || (borg_t - borg_began > 3501 && borg_skill[BI_CDEPTH] == 0))
        return (false);
...
static bool borg_consume(int i)
{
    ...
    case TV_FOOD:
        ...
        if (item->sval == sv_food_ration || item->sval == sv_food_slime_mold
            ...
            if (!borg_skill[BI_ISFULL] && borg_eat_food(item->tval, item->sval))
                return (true);
...
bool borg_crush_junk(void)
{
    ...
    if (borg_danger(c_y, c_x, 1, true, false) > borg_skill[BI_CURHP] / 10)
        return (false);
    ...
    for (i = 0; i < z_info->pack_size; i++) {
        ...
        if (item->tval >= TV_CHEST) {
            ...
            if (item->value > 0
                ...
                && borg_skill[BI_MAXCLEVEL] <= 20 && !item->cursed)
                continue;
...
bool borg_crush_hole(void)
{
    ...
    /* No crush if even slightly dangerous */
    if (borg_skill[BI_CDEPTH]
        && (borg_danger(c_y, c_x, 1, true, false) > borg_skill[BI_CURHP] / 10
            && (borg_skill[BI_CURHP] != borg_skill[BI_MAXHP]
                || borg_danger(c_y, c_x, 1, true, false)
                       > (borg_skill[BI_CURHP] * 2) / 3)))
        return (false);
    ...
    for (i = 0; i < z_info->pack_size; i++) {
        ...
        if (item->tval == TV_WAND && item->sval == sv_wand_teleport_away
            && borg_class == CLASS_WARRIOR && borg_skill[BI_ATPORTOTHER] <= 8)
            continue;
        if (item->tval == TV_ROD
            && (item->sval == sv_rod_light && borg_skill[BI_CURLITE] <= 0))
            continue;
         ...
         */
        if ((item->tval == borg_skill[BI_AMMO_TVAL]) && (value > 0)
            && (borg_skill[BI_AMISSILES] <= 35) && borg_skill[BI_CDEPTH] >= 1) {
            value += 5000L;
        }
...
bool borg_crush_slow(void)
{
    ...
    /* No crush if even slightly dangerous */
    if (borg_danger(c_y, c_x, 1, true, false) > borg_skill[BI_CURHP] / 20)
        return (false);
...
bool borg_dump_quiver(void)
{
    ...
    for (i = QUIVER_END - 1; i >= QUIVER_START; i--) {
        ...
        if (!item->cursed && item->tval == borg_skill[BI_AMMO_TVAL]) {
            ...
            if (borg_skill[BI_AMISSILES] <= quiver_capacity && item->to_d >= 0
                && item->to_h >= 0)
                continue;
        }
...
int borg_count_sell(void)
{
    ...
    if (borg_skill[BI_CLEVEL] < 25)
        greed = (borg_gold / 100L) + 50L;
    if (borg_skill[BI_CLEVEL] < 20)
        greed = (borg_gold / 100L) + 35L;
    if (borg_skill[BI_CLEVEL] < 15)
        greed = (borg_gold / 100L) + 20L;
    if (borg_skill[BI_CLEVEL] < 13)
        greed = (borg_gold / 100L) + 10L;
    if (borg_skill[BI_CLEVEL] < 10)
        greed = (borg_gold / 100L) + 5L;
    if (borg_skill[BI_CLEVEL] < 5)
        greed = (borg_gold / 100L);
...
bool borg_leave_level(bool bored)
{
    ...
    if (!borg_skill[BI_CDEPTH]) {
        ...
        if (borg_skill[BI_MAXDEPTH] == 100 && !borg_cfg[BORG_PLAYS_RISKY]) {
    ...
    /* Power dive if I am playing too shallow*/
             ...
             && NULL == borg_prepared(borg_skill[BI_CDEPTH] + 5)
    ...
    if (borg_scumming_pots && !vault_on_level
        && (borg_skill[BI_AEZHEAL] >= 3 || borg_skill[BI_ALIFE] >= 1)) {
    ...
    if (g < 0) {
        ...
        if (need_restock && borg_skill[BI_CDEPTH] >= 5 && borg_recall()) {
