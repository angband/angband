...
static void borg_think_home_sell_aux2_slow(int n, int start_i)
{
    ...
    if ((n != 0) && !borg_shops[7].ware[n].iqty
        && !borg_shops[7].ware[n - 1].iqty)
        return;
...
static bool borg_think_home_sell_aux(bool save_best)
{
    ...
    /* and pack is full */
    if (borg_shops[7].ware[icky].iqty && borg_items[PACK_SLOTS - 1].iqty)
        return (false);
...
static bool borg_good_buy(borg_item* item, int who, int ware)
{
    ...
    if (who == 6) {
        ...
        if ((item->tval == TV_POTION
            ...
            if (borg_cfg[BORG_SELF_SCUM] && /* borg is allowed to scum */
                ...
                borg_skill[BI_FOOD] + num_food >= 100
                && /* Have plenty of food */
                item->cost <= 85000) /* Its not too expensive */
            {
                if (adj_dex_safe[borg_skill[BI_DEX]] + borg_skill[BI_CLEVEL]
    ...
    if ((player->class->magic.num_books < 4) && borg_skill[BI_MAXCLEVEL] <= 8) {
        if (obj_kind_can_browse(&k_info[item->kind]) && item->sval >= 1)
            return (false);
    }
...
static bool borg_think_shop_buy_aux(void)
{
    ...
    for (k = 0; k < (z_info->store_max - 1); k++) {
        ...
        if ((borg_skill[BI_CURLITE] == 0 || borg_skill[BI_FOOD] == 0) && k != 0
            && k != 7)
            continue;
        if ((borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED]) && k != 3)
            continue;
...
static bool borg_think_home_buy_aux(void)
{
    ...
    for (n = 0; n < z_info->store_inven_max; n++) {
        ...
        if (slot >= 0) {
            ...
            if (slot == INVEN_LEFT) {
                ...
                if (!borg_items[INVEN_LEFT].one_ring) {
                    ...
                    /* Move new item into equipment */
                    memcpy(&borg_items[slot], &safe_shops[7].ware[n],
                        sizeof(borg_item));
                ...
                if (!borg_items[INVEN_RIGHT].one_ring) {
                    ...
                    /* Move new item into equipment */
                    memcpy(&borg_items[INVEN_RIGHT], &safe_shops[7].ware[n],
                        sizeof(borg_item));
        ...
        /* Restore shop item */
        memcpy(
            &borg_shops[7].ware[n], &safe_shops[7].ware[n], sizeof(borg_item));
    ...
    for (n = 0; n < z_info->store_inven_max; n++) {
        ...
        for (p = 0; p < sold_item_num; p++) {
            if (sold_item_tval[p] == item->tval
                && sold_item_sval[p] == item->sval && sold_item_store[p] == 7)
                skip_it = true;
        }
        if (skip_it == true)
            continue;
...
static bool borg_choose_shop(void)
{
    ...
    if (borg_gold < borg_cfg[BORG_MONEY_SCUM_AMOUNT]
        && borg_cfg[BORG_MONEY_SCUM_AMOUNT] != 0 && !borg_skill[BI_CDEPTH]
        && borg_skill[BI_LIGHT] && !borg_cfg[BORG_SELF_SCUM]) {
...
static bool borg_think_dungeon_light(void)
{
    ...
    if (!borg_skill[BI_LIGHT]
        ...
        if ((borg_skill[BI_CURSP] < borg_skill[BI_MAXSP]
                && borg_skill[BI_MAXSP] > 0)
            && (borg_grids[c_y][c_x].info & BORG_GLOW) && !borg_skill[BI_ISWEAK]
        ...
        for (ii = 0; ii < 8; ii++) {
            ...
            if ((ag->feat == FEAT_OPEN || ag->feat == FEAT_BROKEN)
                ...
                if (square_isglow(cave, loc(c_x, c_y)))
                    ag->info |= BORG_GLOW;
                continue;
            }
...
static bool borg_think_stair_scum(bool from_town)
{
    ...
    if (track_more.num
        ...
        if (b_j < 8 || ag->feat == FEAT_MORE || borg_skill[BI_CDEPTH] < 30) {
            /* Note */
...
static bool borg_think_dungeon_munchkin(void)
{
    ...
    if (borg_skill[BI_CDEPTH] <= 10)
        closeness
            += (borg_skill[BI_CLEVEL] - 10) + (10 - borg_skill[BI_CDEPTH]);
    ...
    if ((track_less.num
            && borg_skill[BI_CDEPTH] > borg_cfg[BORG_MUNCHKIN_DEPTH])
        && (safe_place || ag->feat == FEAT_LESS)) {
    ...
    /* Going up */
    if ((track_less.num && borg_skill[BI_CDEPTH] != 1 && safe_place)
        || ag->feat == FEAT_LESS) {
        borg_grid* tmp_ag = &borg_grids[c_y][c_x];
...
static bool borg_think_dungeon_brave(void)
{
    ...
    if (borg_skill[BI_CDEPTH] == 100 && borg_defend(p1))
        return true;
...
bool borg_think_dungeon(void)
{
    ...
    borg_near_monster_type(
        borg_skill[BI_MAXCLEVEL] < 15 ? z_info->max_sight : 12);
    ...
    if (borg_skill[BI_CDEPTH] && (time_this_panel >= 700)) {
        /* Start leaving */
        if (!goal_leaving) {
            /* Note */
    ...
    {
        if ((char*)NULL == borg_prepared(borg_skill[BI_CDEPTH] + 15)
            && /* Prepared */
    ...
    if (track_less.num && borg_skill[BI_CLEVEL] < 10 && !goal_less
        && (char*)NULL != borg_prepared(borg_skill[BI_CDEPTH])) {
        /* Note */
    ...
    if (borg_skill[BI_CLEVEL] < 10 && borg_skill[BI_MAXSP]
        && borg_skill[BI_CURSP] == 0 && borg_no_rest_prep <= 1 && !borg_bless
     ...
     */
    if (borg_skill[BI_CDEPTH] == 0 && borg_skill[BI_CLEVEL] < 6
        && borg_gold < 10 && borg_count_sell() < 5) {
