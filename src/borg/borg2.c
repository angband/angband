...
bool borg_los(int y1, int x1, int y2, int x2)
{
    ...
    if (!square_in_bounds_fully(cave, y1, x1)) return (FALSE);
...
void borg_update_view(void)
{
    ...
    for (d = 1; d <= z; d++)
    {
		if (!square_in_bounds_fully(cave, y+d, x+d)) continue;
    ...
    {
		/* Caution */
		if (!square_in_bounds_fully(cave, y+d, x)) continue;
        ag = &borg_grids[y+d][x];
        ag->info |= BORG_XTRA;
    ...
    {
		/* Caution */
		if (!square_in_bounds_fully(cave, y, x+d)) continue;
        ag = &borg_grids[y][x+d];
        ag->info |= BORG_XTRA;
