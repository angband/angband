...
void borg_delete_kill(int i)
{
    ...
    if (goal == GOAL_KILL && borg_flow_y[0] == kill->y && borg_flow_x[0] == kill->x)
        goal = 0;
...
static int borg_new_kill(unsigned int r_idx, int y, int x)
{
    ...
    /* Wipe goals only if I have some light source */
    if (borg_skill[BI_CURLITE] && borg_los(kill->y, kill->x, c_y, c_x))
        goal = 0;
...
static bool observe_kill_move(int y, int x, int d, uint8_t a, wchar_t c, bool flag)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++) {
        ...
        if (!borg_skill[BI_ISIMAGE] && c != r_ptr->d_char)
            continue;
        ...
        if (a != r_ptr->d_attr || borg_skill[BI_ISIMAGE]) {
            ...
            if (!rf_has(r_ptr->flags, RF_ATTR_MULTI) && !rf_has(r_ptr->flags, RF_ATTR_CLEAR)) {
                /* Require flag */
                if (!flag)
                    continue;
...
static int borg_fear_spell(int i)
{
    ...
    /* Damage taken */
    if (borg_oldchp > borg_skill[BI_CURHP])
        ouch = (borg_oldchp - borg_skill[BI_CURHP]) * 2;
    ...
    for (dy = 0; dy < SCREEN_HGT; dy++) {
        ...
        for (dx = 0; dx < SCREEN_WID; dx++) {
            ...
            /* Walls */
                ...
                if (ag->feat == FEAT_PERM && borg_skill[BI_CDEPTH] && x && y && x != (cave->width - 1) && y != (cave->height - 1)) {
                    vault_on_level = true;
                }
            }
            /* lava */
            ...
            /* Doors */
                ...
                if (borg_skill[BI_CLEVEL] <= 5) {
                    ...
                    for (i = 0; i < track_closed.num; i++) {
                        ...
                        if ((track_closed.x[i] == x) && (track_closed.y[i] == y))
                            break;
                    }
...
void borg_update(void)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++) {
        ...
        if (borg_t - kill->when < 2000 && !borg_skill[BI_ISIMAGE])
            continue;
    ...
    if (old_depth != borg_skill[BI_CDEPTH]) {
        ...
        for (u_i = 1; u_i < (unsigned int)(z_info->r_max - 1); u_i++) {
            ...
            if (r_ptr->level > borg_skill[BI_MAXDEPTH])
                continue;
    ...
    if (!borg_skill[BI_KING] && morgoth_on_level) {
        ...
        if (c_y >= 15 && c_y <= AUTO_MAX_Y - 15 && c_x >= 50 && c_x <= AUTO_MAX_X - 50) {
            /* Scan neighbors */
            for (j = 0; j < 24; j++) {
    ...
    if (borg_skill[BI_CLEVEL] <= 5 && borg_skill[BI_CDEPTH] && track_closed.num) {
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        if (wank->is_kill && observe_kill_move(wank->y, wank->x, 0, wank->t_a, wank->t_c, false)) {
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        if (wank->is_take && observe_take_move(wank->y, wank->x, 0, wank->t_a, wank->t_c)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        if (wank->is_kill && observe_kill_move(wank->y, wank->x, 1, wank->t_a, wank->t_c, false)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        /* Track moving monsters */
        if (wank->is_kill && observe_kill_move(wank->y, wank->x, 2, wank->t_a, wank->t_c, false)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        /* Track moving monsters */
        if (wank->is_kill && observe_kill_move(wank->y, wank->x, 3, wank->t_a, wank->t_c, false)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        /* Track moving monsters */
        if (wank->is_kill && observe_kill_move(wank->y, wank->x, 7, wank->t_a, wank->t_c, true)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        /* Track new objects */
        if (wank->is_take && observe_take_diff(wank->y, wank->x, wank->t_a, wank->t_c)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--) {
        ...
        /* Track new monsters */
        if (wank->is_kill && observe_kill_diff(wank->y, wank->x, wank->t_a, wank->t_c)) {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    }
...
void borg_react(const char* msg, const char* buf)
{
    ...
    if (borg_dont_react || borg_skill[BI_ISPARALYZED]) {
