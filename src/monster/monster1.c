...
void get_attack_colors(int melee_colors[RBE_MAX], int spell_colors[RSF_MAX])
{
	...
	for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if (i >= INVEN_WIELD && (!known || o_ptr->to_a > 0 ||
				...
				!check_state(OF_RES_DISEN, p_ptr->state.flags))
		{
	...
	/* Acid */
	if (check_state(OF_IM_ACID, p_ptr->state.flags))
		tmp_col = TERM_L_GREEN;
	else if (check_state(OF_RES_ACID, p_ptr->state.flags))
		tmp_col = TERM_YELLOW;
	else
		tmp_col = TERM_ORANGE;
	...
	/* Cold and ice */
	if (check_state(OF_IM_COLD, p_ptr->state.flags))
		tmp_col = TERM_L_GREEN;
	else if (check_state(OF_RES_COLD, p_ptr->state.flags))
		tmp_col = TERM_YELLOW;
	else
		tmp_col = TERM_ORANGE;
	...
	/* Elec */
	if (check_state(OF_IM_ELEC, p_ptr->state.flags))
		tmp_col = TERM_L_GREEN;
	else if (check_state(OF_RES_ELEC, p_ptr->state.flags))
		tmp_col = TERM_YELLOW;
	else
		tmp_col = TERM_ORANGE;
	...
	/* Fire */
	if (check_state(OF_IM_FIRE, p_ptr->state.flags))
		tmp_col = TERM_L_GREEN;
	else if (check_state(OF_RES_FIRE, p_ptr->state.flags))
		tmp_col = TERM_YELLOW;
	else
		tmp_col = TERM_ORANGE;
	...
	/* Nexus  */
	if (!check_state(OF_RES_NEXUS, p_ptr->state.flags))
	{
	...
	/* Nether */
	if (!check_state(OF_RES_NETHR, p_ptr->state.flags))
	{
	...
	if (!check_state(OF_RES_SOUND, p_ptr->state.flags))
	{
	...
	else
	{
 	...
 	/* Shards */
 	if(!check_state(OF_RES_SHARD, p_ptr->state.flags))
	...
	/* Water */
	if (!check_state(OF_RES_CONFU, p_ptr->state.flags) || !check_state(OF_RES_SOUND, p_ptr->state.flags))
	{
	...
	if (st.skills[SKILL_SAVE] < 100)
	{
		...
		/* Mind blast */
		spell_colors[RSF_MIND_BLAST] = (check_state(OF_RES_CONFU, p_ptr->state.flags) ?
			TERM_YELLOW : TERM_ORANGE);

		/* Brain smash slows even when conf/blind resisted */
		spell_colors[RSF_BRAIN_SMASH] = (check_state(OF_RES_BLIND, p_ptr->state.flags) &&
				check_state(OF_FREE_ACT, p_ptr->state.flags) && check_state(OF_RES_CONFU, p_ptr->state.flags)
	...
	if (!check_state(OF_SUST_STR, p_ptr->state.flags)) melee_colors[RBE_LOSE_STR] = TERM_ORANGE;
	if (!check_state(OF_SUST_INT, p_ptr->state.flags)) melee_colors[RBE_LOSE_INT] = TERM_ORANGE;
	if (!check_state(OF_SUST_WIS, p_ptr->state.flags)) melee_colors[RBE_LOSE_WIS] = TERM_ORANGE;
	if (!check_state(OF_SUST_DEX, p_ptr->state.flags)) melee_colors[RBE_LOSE_DEX] = TERM_ORANGE;
	if (!check_state(OF_SUST_CON, p_ptr->state.flags)) melee_colors[RBE_LOSE_CON] = TERM_ORANGE;
	if (!check_state(OF_SUST_CHR, p_ptr->state.flags)) melee_colors[RBE_LOSE_CHR] = TERM_ORANGE;

	/* Drain all gets a red warning */
	if (!check_state(OF_SUST_STR, p_ptr->state.flags) || !check_state(OF_SUST_INT, p_ptr->state.flags) ||
			!check_state(OF_SUST_WIS, p_ptr->state.flags) || !check_state(OF_SUST_DEX, p_ptr->state.flags) ||
			!check_state(OF_SUST_CON, p_ptr->state.flags) || !check_state(OF_SUST_CHR, p_ptr->state.flags))
	...
	melee_colors[RBE_EXP_10] = melee_colors[RBE_EXP_20] =
		...
		check_state(OF_HOLD_LIFE, p_ptr->state.flags) ? TERM_YELLOW : TERM_ORANGE;
	...
	if (!check_state(OF_RES_NEXUS, p_ptr->state.flags) && st.skills[SKILL_SAVE] < 100)
