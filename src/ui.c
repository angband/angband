							...
							int row, int col, int width)
{
	...
	display_event_aux(&evts[oid], menu->target.self.object_id, color, row, col, width);
}
...
{
	int oid = cursor;
	if(cursor < 0 || cursor >= menu->filter_count) return FALSE;
	if(menu->object_list) {
		oid = menu->object_list[cursor]; 
	}
	if(!menu->row_funcs->valid_row) return TRUE;
...
static int get_cursor_key(menu_type *menu, int top, char key)
{
	int i;
	int n = menu->filter_count;
	if(menu->flags & MN_NO_TAGS) return -1;
		...
		for(i = 0; menu->selections[i] ; i++)
			if(menu->selections[i] == key) return i;
	}
	...
	switch(in->type)
	{
		...
		case EVT_KBRD:
		{
			...
			if((menu->cmd_keys && strchr(menu->cmd_keys, in->key))
														|| in->key == ESCAPE)
			{
				if(menu->flags & MN_NO_ACT) return FALSE;
				else return handle_menu_key(in->key, menu, *cursor);
			}
				...
				menu_iter_id iter_id, const region *loc)
{
	...
	if(!iter || !skin) {
		msg_print(format("could not find menu VTAB (%d, %d)!", skin_id, iter_id));
		return FALSE;
	}
