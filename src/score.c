...
static void highscore_write(const high_score scores[], size_t sz)
{
	...
	{
		msg("%s", "Lock file in place for scorefile; not writing.");
		return;
	}
	...
	if (!lok)
	{
		msg("%s", "Failed to create lock for scorefile; not writing.");
		return;
	}
	...
	if (!scorefile)
	{
		msg("%s", "Failed to open new scorefile for writing.");
	...
	if (file_exists(old_name) && !file_delete(old_name))
		msg("%s", "Couldn't delete old scorefile");

	if (file_exists(cur_name) && !file_move(cur_name, old_name))
		msg("%s", "Couldn't move old scores.raw out of the way");

	if (!file_move(new_name, cur_name))
		msg("%s", "Couldn't rename new scorefile to scores.raw");
...
void enter_score(time_t *death_time)
{
	...
	for (j = OPT_SCORE; j < OPT_SCORE + N_OPTS_CHEAT; ++j)
	{
		...
		msg("%s", "Score not registered for cheaters.");
		message_flush();
		return;
	}
	...
	{
		msg("%s", "Score not registered for wizards.");
		message_flush();
	}
	...
	{
		msg("%s", "Score not registered for borgs.");
		message_flush();
	}
	...
	{
		msg("%s", "Score not registered due to interruption.");
		message_flush();
	}
	...
	{
		msg("%s", "Score not registered due to quitting.");
		message_flush();
	}
